 ...
 */ import { serve } from "https://deno.land/std@0.168.0/http/server.ts";
  ...
  */ async function detectIntent(text) {
   ...
   // По умолчанию считаем едой если:
   // - есть запятые (перечисление продуктов)
   // - короткое сообщение без вопросов
   if (lowerText.includes(',') || lowerText.length < 50) {
     return 'food';
   }
   // Если непонятно - считаем вопросом (безопаснее)
   return 'question';
 }
  ...
  */ async function handleTextMessage(message) {
   ...
   if (stateData.state === 'waiting_name') {
     ...
     try {
       const { data: profile } = await supabase.from('user_profiles').select('*').eq('user_id', user.id).single();
       const { data: currentPlan } = await supabase.from('nutrition_plans').select('*').eq('user_id', user.id).eq('is_active', true).single();
     ...
     const response = await fetch('https://api.openai.com/v1/chat/completions', {
       ...
       body: JSON.stringify({
         ...
         messages: [
           {
             role: 'system',
             content: 'Ты C.I.D. - AI-диетолог. Анализируешь питание и даешь рекомендации.'
           },
           {
             role: 'user',
             content: prompt
 ...
 2. Запрашивай уточнение ТОЛЬКО если веса РЕАЛЬНО отсутствуют
 3. ⚠️ КРИТИЧНО: Если НЕСКОЛЬКО продуктов без веса - спроси про ВСЕ сразу в ОДНОМ вопросе!
 4. ❌ НИКОГДА не придумывай вес сам - если вес не указан, ОБЯЗАТЕЛЬНО спроси!
 
 Примеры:
 - "банан 150г, яблоко 200г" → все веса есть, не спрашивай
 - "банан, яблоко" → спроси: "Укажите вес для банана и яблока"
 - "банан, клубника, овсянка 60г" → спроси: "Укажите вес для банана и клубники"`;
     const prompt = `Ты - C.I.D., AI-диетолог. Проанализируй прием пищи клиента.
 ...
 КРИТИЧЕСКИ ВАЖНО: 
 - Если в описании есть цифры с "г", "мл", "кг" рядом с продуктом - это УЖЕ указанный вес, используй его!
 - Примеры с указанным весом: "банан 400г", "овсянка 60г", "молоко 200мл"
 - Примеры БЕЗ веса: "банан", "тарелка супа", "порция курицы"
 
 ⚠️ ЗАПРЕЩЕНО придумывать вес самостоятельно!
 - Если у продукта нет веса - установи need_clarification: true
 - Если у НЕСКОЛЬКИХ продуктов нет веса - спроси про ВСЕ в одном вопросе
 - Пример вопроса: "Укажите примерный вес для банана и замороженной клубники"
     ...
     await sendMessage(chatId, resultText, {
       inline_keyboard: [
         [
           {
             text: "✏️ Изменить",
             callback_data: `edit_meal_${savedLog.id}`
           },
           {
             text: "🗑 Удалить",
             callback_data: `delete_meal_${savedLog.id}`
           }
         ],
         [
           {
  ...
  */ async function handleRecipeRequest(userId, chatId, dbUserId, request) {
   try {
     ...
     const response = await fetch('https://api.openai.com/v1/chat/completions', {
       ...
       body: JSON.stringify({
         ...
         messages: [
           {
             role: 'system',
             content: 'Ты C.I.D. - AI-диетолог. Помогаешь с рецептами и планированием питания.'
           },
           {
             role: 'user',
             content: prompt
