     ...
     const response = await fetch('https://api.openai.com/v1/chat/completions', {
       ...
       body: JSON.stringify({
         ...
         messages: [
           {
             role: 'system',
             content: 'Ты C.I.D. - AI-диетолог. Анализируешь питание и даешь рекомендации. ОБЯЗАТЕЛЬНО используй стандартные таблицы БЖУ для точных расчетов. Будь последовательным - одинаковые продукты всегда имеют одинаковую калорийность на 100г.'
           },
           {
             role: 'user',
             content: prompt
