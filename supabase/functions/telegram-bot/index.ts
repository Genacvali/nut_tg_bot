...
Сфотографируй еду - я автоматически распознаю продукты и рассчитаю КБЖУ (требуется подписка)
...
async function onboardingStep3(chatId: number, userId: number) {
  await sendMessage(
    ...
    `• Я автоматически распознаю продукты и КБЖУ!\n\n` +
    `🤖 **Я автоматически:**\n` +
...
async function onboardingStep6(chatId: number, userId: number) {
  await sendMessage(
    ...
    `• 📸 Фотографируй еду для быстрого распознавания\n` +
    `• Не забывай про воду!\n` +
