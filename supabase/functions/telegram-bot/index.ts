...
async function detectIntent(text: string): Promise<'food' | 'question'> {
  ...
  const foodPatterns = [
    ...
    /\d+\s*(г|грам|мл|кг|шт)/i,  // есть граммы/мл/кг/шт
...
💡 Все параметры рассчитаны с помощью ИИ и максимально приближены к реальности, но помни — это ориентиры для старта. Слушай своё тело и корректируй план по мере необходимости.`
    ...
    if (subscriptionInfo && subscriptionInfo.is_trial) {
      ...
      const trialMessage = `🎁 **Пробный период: ${daysLeft} дней**\n\n` +
        `💡 Сейчас тебе ничего вводить не нужно - **никаких данных карт**!\n\n` +
        `Пользуйся всеми функциями бота **совершенно бесплатно** ${daysLeft} дней.\n\n` +
        `После истечения пробного периода будет доступна оплата:\n` +
        `📦 1 месяц - 199₽\n` +
        `📦 3 месяца - 499₽\n` +
        `📦 1 год - 1990₽\n\n` +
