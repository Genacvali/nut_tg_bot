...
class ConversationManager {
   ...
   */ static async getCurrentTopic(userId) {
    try {
      const { data, error } = await supabase.rpc('get_conversation_topic', {
        p_user_id: userId
      });
      if (error) {
   ...
   */ static async checkAndClearIfStale(userId) {
 ...
 */ async function detectIntentWithContext(text, userId, history) {
  try {
    // Формируем контекст из истории
    const contextMessages = history.slice(-5).map((msg)=>`${msg.role === 'user' ? 'Пользователь' : 'Ассистент'}: ${msg.content}`).join('\n');
...
async function detectIntent(text) {
  ...
  const waterPatterns = [
    /\d+\s*(л|литр|мл|миллилитр)\b/i,
    /(выпил|выпила|попил|попила|пью|пьёт)\s+(вод|жидкост)/i,
    /^\s*(вод|жидкост)/i,
    /(стакан|бутылк|кружк|чашк)\s+(вод|жидкост)/i,
...
async function getUserState(userId) {
  const { data, error } = await supabase.from('user_states').select('*').eq('telegram_id', userId).maybeSingle();
  if (error) {
...
function validateNutrition(analysis) {
  const notes = [];
  let corrected = false;
  ...
  function validateProduct(item) {
    ...
    const calcKcal = item.protein * KCAL_PER_G.protein + item.fats * KCAL_PER_G.fat + item.carbs * KCAL_PER_G.carb;
    ...
    if (category === 'nuts') {
      ...
      if (per100.fats < 45 || per100.calories < 500 || per100.carbs > 20) {
        ...
        fixed.calories = Math.round(fixed.protein * KCAL_PER_G.protein + fixed.fats * KCAL_PER_G.fat + fixed.carbs * KCAL_PER_G.carb);
      }
    ...
    const finalCalcKcal = fixed.protein * KCAL_PER_G.protein + fixed.fats * KCAL_PER_G.fat + fixed.carbs * KCAL_PER_G.carb;
  ...
  const totalCalcKcal = totals.protein * KCAL_PER_G.protein + totals.fats * KCAL_PER_G.fat + totals.carbs * KCAL_PER_G.carb;
...
async function getChatHistory(dbUserId, limit = 10) {
  try {
    const { data, error } = await supabase.from('conversation_history').select('role, content').eq('user_id', dbUserId).order('created_at', {
      ascending: false
    }).limit(limit);
    if (error) {
...
async function clearChatHistory(dbUserId) {
  try {
    const { error } = await supabase.from('conversation_history').delete().eq('user_id', dbUserId);
    if (error) {
...
async function saveUserPreference(dbUserId, preferenceType, item, description) {
  try {
...
async function getUserPreferences(dbUserId) {
  try {
    const { data, error } = await supabase.from('user_preferences').select('*').eq('user_id', dbUserId);
    if (error) {
 ...
 */ async function deleteUserPreference(dbUserId, preferenceType, item) {
  try {
    const { error } = await supabase.from('user_preferences').delete().eq('user_id', dbUserId).eq('preference_type', preferenceType).eq('item', item.toLowerCase());
    if (error) {
 ...
 */ async function extractPreferencesFromText(text) {
  ...
  const uniquePreferences = preferences.filter((pref, index, self)=>index === self.findIndex((p)=>p.type === pref.type && p.item === pref.item));
  return uniquePreferences;
}
/**
 * Получить информацию о подписке пользователя
 */ async function getSubscriptionInfo(dbUserId) {
  try {
 ...
 */ async function checkRateLimit(dbUserId, maxRequests = 30, windowMinutes = 1) {
  try {
 ...
 */ async function saveToCache(cacheKey, cacheType, requestData, responseData, ttlSeconds = 2592000 // 30 дней по умолчанию
) {
  try {
    const { error } = await supabase.rpc('save_to_cache', {
      p_cache_key: cacheKey,
 ...
 */ async function callOpenAIWithRetry(url, options, maxRetries = 3, timeout = 30000) {
  let lastError;
 ...
 */ async function getUserFullContext(telegramId) {
  try {
 ...
 * @returns Полный контекст пользователя или null при ошибке
 */ async function getUserFullContextById(userId) {
  try {
    const { data, error } = await supabase.rpc('get_user_full_context_by_id', {
      p_user_id: userId
    });
    if (error) {
 ...
 */ async function sendMessage(chatId, text, replyMarkup, parseMode = 'Markdown', replyToMessageId) {
  const payload = {
    chat_id: chatId,
    text: text
  };
 ...
 */ async function generateNutritionPlan(profileData) {
  const activityNames = {
...
- Активность: ${activityNames[profileData.activity_level]}
- Цель: ${goalNames[profileData.goal]}
...
async function adjustNutritionPlan(currentPlan, userRequest, profileData) {
 ...
 */ function formatNutritionCard(plan, profileData) {
 ...
 */ function activityKeyboard() {
  return {
    inline_keyboard: [
      [
        {
          text: "🪑 Маленькая (Не тренируюсь вообще)",
          callback_data: "activity_low"
        }
      ],
      [
        {
          text: "🚶 Средняя (1-2 раза в неделю)",
          callback_data: "activity_medium"
        }
      ],
      [
        {
          text: "💪 Высокая (3+ раза в неделю)",
  ...
  return {
    inline_keyboard: [
      [
        {
          text: "250 мл",
          callback_data: "log_water_250"
        },
        {
          text: "500 мл",
          callback_data: "log_water_500"
        }
      ],
      [
        {
          text: "1 л",
          callback_data: "log_water_1000"
        },
        {
          text: "✏️ Другое",
          callback_data: "log_water_custom"
        }
      ],
      [
        {
 ...
 */ function aiResponseActionsKeyboard(hasMultipleItems = false) {
  const keyboard = [];
  if (hasMultipleItems) {
    ...
    keyboard.push([
      {
        ...
        callback_data: "save_ai_meal_plan"
      }
    ]);
    keyboard.push([
      {
        text: "💾 Сохранить по отдельности",
        callback_data: "save_ai_items_separately"
      }
    ]);
  } else {
    ...
    keyboard.push([
      {
        ...
        callback_data: "save_ai_recipe"
      }
    ]);
  }
  keyboard.push([
    {
      text: "🍽 Записать как прием",
      callback_data: "log_ai_as_meal"
    }
  ]);
  keyboard.push([
    {
 ...
 */ function afterFoodLogKeyboard(mealId) {
  const keyboard = [
    [
      {
        text: "🍽 Еще прием",
        callback_data: "quick_log_food"
      },
      {
        text: "📊 Мой день",
        callback_data: "diary"
  ...
  if (mealId) {
    keyboard.push([
      {
        text: "⭐ В избранное",
        callback_data: `save_template_${mealId}`
      },
      {
        text: "✏️ Изменить",
        callback_data: `edit_meal_${mealId}`
      }
    ]);
    keyboard.push([
      {
        text: "🗑 Удалить",
        callback_data: `delete_meal_${mealId}`
      }
    ]);
  }
  keyboard.push([
  ...
  return {
    inline_keyboard: [
      [
        {
          text: "📝 Мои приемы",
          callback_data: "manage_meals"
        },
        {
          text: "⚡ Быстрый лог",
          callback_data: "quick_log"
        }
      ],
      [
        {
...
Готов начать? 🚀`;
  ...
  const { data: todayLogs } = await supabase.from('food_logs').select('*').eq('user_id', user.id).gte('logged_at', `${today}T00:00:00`);
  // Считаем съеденное
 ...
 */ async function handleCallbackQuery(callbackQuery) {
  ...
  if (data === 'fill_profile') {
    ...
    await editMessageText(chatId, messageId, "✨ Отлично! Давай познакомимся.\n\n📝 Как тебя зовут?");
    ...
    if (stateData) {
      ...
      await setUserState(userId, 'waiting_age', {
        gender
      });
      await sendMessage(chatId, "🎂 Сколько тебе лет?");
    }
    ...
    if (stateData) {
      ...
      // Если состояния нет, создаем заново
      await setUserState(userId, 'waiting_goal', {
    ...
    await sendMessage(chatId, "✏️ Что хочешь изменить?", {
      inline_keyboard: [
        [
          {
            text: "📊 КБЖУ + Вода",
            callback_data: "edit_nutrition"
          }
        ],
        [
          {
            text: "👤 Параметры профиля",
            callback_data: "edit_parameters"
          }
        ]
      ]
    });
  } else if (data === 'edit_nutrition') {
    await sendMessage(chatId, "📊 Что конкретно хочешь изменить в КБЖУ?", {
      inline_keyboard: [
        [
          {
            text: "🔥 Калории",
            callback_data: "edit_calories"
          }
        ],
        [
          {
            text: "🥩 Белки",
            callback_data: "edit_protein"
          }
        ],
        [
          {
            text: "🥑 Жиры",
            callback_data: "edit_fats"
          }
        ],
        [
          {
            text: "🍞 Углеводы",
            callback_data: "edit_carbs"
          }
        ],
        [
          {
            text: "💧 Вода",
            callback_data: "edit_water"
          }
        ]
      ]
    });
  } else if (data === 'edit_parameters') {
    await sendMessage(chatId, "👤 Какой параметр хочешь изменить?", {
      inline_keyboard: [
        [
          {
            text: "📝 Имя",
...
Можешь написать примерно: "тарелка супа" или "рис с мясом" — я уточню детали.`, {
    ...
    await sendMessage(chatId, `💬 **Режим диалога активирован**
...
Задавай вопросы! 😊`, {
    ...
    try {
      const { data: templates, error } = await supabase.rpc('get_user_meal_templates', {
        p_user_id: user.id,
        p_limit: 10
      });
      if (error) throw error;
      if (!templates || templates.length === 0) {
        await sendMessage(chatId, `⭐ **Быстрый лог**\n\n` + `У тебя пока нет сохраненных шаблонов.\n\n` + `💡 Залогируй еду и нажми "⭐ В избранное" чтобы создать шаблон!`, {
          inline_keyboard: [
            [
              {
      ...
      await sendMessage(chatId, templatesList, {
        inline_keyboard: keyboard
      });
    } catch (error) {
    ...
    try {
      ...
      const { data: result, error } = await supabase.rpc('use_meal_template', {
        p_user_id: user.id,
    ...
    if (meal) {
      await setUserState(userId, 'editing_meal', {
        mealId: mealId,
        originalDescription: meal.description
      });
      await sendMessage(chatId, `✏️ **Редактирование приема пищи**\n\n` + `**Текущее описание:**\n${meal.description}\n\n` + `**Текущие КБЖУ:**\n🔥 ${meal.calories}ккал | Б:${meal.protein}г | Ж:${meal.fats}г | У:${meal.carbs}г\n\n` + `Напиши или наговори новое описание приема пищи:`, {
        inline_keyboard: [
          [
            {
              text: "❌ Отмена",
    ...
    await sendMessage(chatId, `📊 **Изменение плана КБЖУ**\n\n` + `Я могу пересчитать твой план на основе текущих параметров или ты можешь ввести желаемые значения вручную.\n\n` + `Что ты хочешь сделать?`, {
      inline_keyboard: [
        [
          {
            text: "🔄 Пересчитать автоматически",
            callback_data: "recalculate_nutrition"
          }
        ],
        [
          {
            text: "✏️ Ввести вручную",
            callback_data: "manual_nutrition"
          }
        ],
        [
          {
    ...
    await sendMessage(chatId, `✏️ **Изменение параметров**\n\n` + `Выбери, что хочешь изменить:`, {
      inline_keyboard: [
        [
          {
            text: "⚖️ Вес",
    ...
    await sendMessage(chatId, `⚖️ **Изменение веса**\n\nВведи свой новый вес в килограммах:`, {
      inline_keyboard: [
        [
          {
            text: "❌ Отмена",
    ...
    await sendMessage(chatId, `📏 **Изменение роста**\n\nВведи свой рост в сантиметрах:`, {
      inline_keyboard: [
        [
          {
            text: "❌ Отмена",
    ...
    await sendMessage(chatId, `🎂 **Изменение возраста**\n\nВведи свой возраст в годах:`, {
      inline_keyboard: [
        [
          {
            text: "❌ Отмена",
    ...
    await sendMessage(chatId, `🏃 **Изменение уровня активности**\n\nВыбери свой уровень активности:`, activityKeyboard());
    ...
    try {
      const { data: result, error } = await supabase.rpc('log_water_intake', {
        p_user_id: user.id,
      ...
      await sendMessage(chatId, messageText, {
        inline_keyboard: [
          [
            {
              text: "💧 Еще воды",
              callback_data: "quick_log_water"
            },
            {
              text: "📊 Статистика",
              callback_data: "water_stats"
            }
          ],
          [
            {
    ...
    await sendMessage(chatId, `💧 **Сколько воды выпил?**\n\nВыбери количество:`, {
      inline_keyboard: [
        [
          {
            text: "250 мл",
    ...
    try {
      const { data: stats } = await supabase.rpc('get_water_stats_today', {
        p_user_id: user.id
      });
      if (stats) {
    ...
    await sendMessage(chatId, `🛒 **Список покупок**\n\n` + `Я могу составить список покупок на основе твоего плана питания.\n\n` + `На сколько дней составить список?`, {
      inline_keyboard: [
        [
          {
            text: "📅 На 3 дня",
            callback_data: "shopping_list_3"
          },
          {
            text: "📅 На 7 дней",
            callback_data: "shopping_list_7"
          }
        ],
        [
          {
            text: "📅 На 14 дней",
            callback_data: "shopping_list_14"
          }
        ],
        [
          {
    ...
    await sendMessage(chatId, `📈 **Мой прогресс**\n\n` + `Выбери график для просмотра:`, {
      inline_keyboard: [
        [
          {
            text: "🔥 Калории",
            callback_data: "chart_calories"
          },
          {
            text: "🥩 Белок",
            callback_data: "chart_protein"
          }
        ],
        [
          {
    ...
    const { data: meal, error } = await supabase.from('food_logs').insert({
      user_id: user.id,
    ...
    await sendMessage(chatId, `✅ **Прием записан!**\n\n` + `📊 КБЖУ: ${calories} ккал | Б: ${protein}г | Ж: ${fats}г | У: ${carbs}г\n\n` + `Что дальше?`, afterFoodLogKeyboard(meal.id));
  }
}
/**
 ...
 */ async function handleTextMessage(message) {
    ...
    if (intent === 'food') {
      ...
      if (extractedPrefs.length > 0) {
        for (const pref of extractedPrefs){
          await saveUserPreference(user.id, pref.type, pref.item);
        }
      ...
      await sendMessage(message.chat.id, `❓ Используй кнопки меню для навигации`, getMainKeyboard());
      return;
    }
  }
  // Ожидание имени
  if (stateData.state === 'waiting_name') {
    ...
    await setUserState(userId, stateData.state, stateData.data);
    await sendMessage(message.chat.id, `Приятно познакомиться, ${message.text}! 👋\n\n👤 Укажи свой пол:`, genderKeyboard());
    ...
    await setUserState(userId, stateData.state, stateData.data);
    await sendMessage(message.chat.id, "💪 Выбери уровень активности:", activityKeyboard());
    ...
    try {
      ...
      const { error: deactivateError } = await supabase.from('nutrition_plans').update({
        is_active: false
      }).eq('user_id', user.id);
      if (deactivateError) {
      ...
      const { data: savedPlan, error: planError } = await supabase.from('nutrition_plans').insert({
        user_id: user.id,
    ...
    try {
      ...
      const { data: updatedPlan } = await supabase.from('nutrition_plans').select('*').eq('id', currentPlan.id).single();
    ...
    try {
      const { data: result, error } = await supabase.rpc('create_meal_template_from_log', {
        p_user_id: user.id,
    ...
    try {
      ...
      const { data: result, error } = await supabase.rpc('log_weight', {
        p_user_id: user.id,
        p_weight: weight,
        p_note: null
      });
      console.log('log_weight response:', {
        result,
        error
      });
      if (error) {
    ...
    await clearUserState(userId);
    await sendMessage(message.chat.id, `✅ **Рецепт сохранен!**\n\n` + `📖 "${recipeName}"\n\n` + `Теперь ты можешь найти его в **Моё меню → Мои рецепты**`, {
      inline_keyboard: [
        [
          {
            text: "📖 Мои рецепты",
    ...
    await clearUserState(userId);
    await sendMessage(message.chat.id, `✅ **Рацион сохранен!**\n\n` + `📖 "${mealPlanName}"\n\n` + `Теперь ты можешь найти его в **Моё меню → Мои рецепты**`, {
      inline_keyboard: [
        [
          {
            text: "📖 Мои рецепты",
 ...
 */ async function handleNavigationButtons(message, user) {
  ...
  switch(text){
    ...
    case '📖 Моё меню':
      await sendMessage(chatId, `📖 **Моё меню**\n\n` + `Здесь хранятся твои сохраненные шаблоны и рецепты.\n\n` + `**🍽 Мои шаблоны** - быстрое логирование повторяющихся приемов пищи\n` + `**📖 Мои рецепты** - сохраненные рецепты с инструкциями\n` + `**🛒 Список покупок** - автоматический список на основе рациона`, myMenuKeyboard());
      break;
    // 🔥 Кнопка: Профиль
    case '👤 Профиль':
...
Верни JSON:
{
  ...
  "comment": "краткий комментарий"
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'Ты C.I.D. - AI-диетолог. КРИТИЧЕСКИ ВАЖНО: СТРОГО используй ТОЛЬКО таблицы БЖУ из инструкций для расчетов. НЕ придумывай значения. Для орехов ВСЕГДА: ~620 ккал/100г, Ж:55г (МНОГО жиров!), У:12г (мало углеводов!). Пример: 70г орехов = 434 ккал, Ж:38.5г, У:8.4г. Будь математически точным при умножении на вес.'
          },
          {
            role: 'user',
            content: prompt
    ...
    }).eq('id', mealId).eq('user_id', dbUserId);
    if (error) {
      throw error;
    }
 ...
 */ async function handleParameterEdit(userId, chatId, dbUserId, param, value) {
  try {
    if ([
      'calories',
      'protein',
      ...
      // Редактирование КБЖУ
      const { data: currentPlan } = await supabase.from('nutrition_plans').select('*').eq('user_id', dbUserId).eq('is_active', true).single();
      ...
      await supabase.from('nutrition_plans').update(updates).eq('id', currentPlan.id);
      // Получаем обновленный план
      const { data: updatedPlan } = await supabase.from('nutrition_plans').select('*').eq('id', currentPlan.id).single();
      const { data: profile } = await supabase.from('user_profiles').select('*').eq('user_id', dbUserId).single();
      const cardText = formatNutritionCard(updatedPlan, profile);
      ...
      await supabase.from('user_profiles').update(updates).eq('user_id', dbUserId);
 ...
 */ async function handleWaterLogging(userId, chatId, dbUserId, text) {
  try {
    ...
    if (amountMl === 0 || amountMl > 5000) {
      await sendMessage(chatId, `💧 **Сколько воды ты выпил?**\n\n` + `Укажи количество:\n` + `• В миллилитрах: "500 мл"\n` + `• В литрах: "1.5 л"\n` + `• Или просто: "стакан", "бутылка"`, {
        inline_keyboard: [
          [
            {
              text: "250 мл",
...
async function handleFoodLogging(userId, chatId, dbUserId, foodDescription, clarificationAttempt = 0) {
  try {
    ...
    const today = new Date().toISOString().split('T')[0];
    const { data: todayLogs } = await supabase.from('food_logs').select('*').eq('user_id', dbUserId).gte('logged_at', `${today}T00:00:00`).order('logged_at', {
      ascending: false
    });
...
Верни JSON:
{
  ...
  "comment": "краткая фраза"
    ...
    if (analysis.breakdown && Array.isArray(analysis.breakdown)) {
      const filteredBreakdown = analysis.breakdown.filter((item)=>{
      ...
      const recalculated = filteredBreakdown.reduce((sum, item)=>({
    ...
    const { data: savedLog, error: saveError } = await supabase.from('food_logs').insert({
      user_id: dbUserId,
      description: foodDescription,
      calories: analysis.calories,
      protein: analysis.protein,
 ...
 */ async function handleRecipeRequest(userId, chatId, dbUserId, request, messageId) {
  try {
    ...
    if (!rateLimit.allowed) {
      ...
      await sendMessage(chatId, `⏱ Слишком много запросов! Пожалуйста, подожди ${rateLimit.retryAfter} секунд.\n\nЭто защита от спама для всех пользователей. 🙏`, undefined, 'Markdown', messageId);
      return;
    }
    ...
    console.log(`Found ${extractedPrefs.length} preferences:`, extractedPrefs);
    for (const pref of extractedPrefs){
      await saveUserPreference(dbUserId, pref.type, pref.item);
    }
    ...
    const { data: plan } = await supabase.from('nutrition_plans').select('*').eq('user_id', dbUserId).eq('is_active', true).single();
    const today = new Date().toISOString().split('T')[0];
    const { data: todayLogs } = await supabase.from('food_logs').select('*').eq('user_id', dbUserId).gte('logged_at', `${today}T00:00:00`);
    // Считаем съеденное
    const consumed = todayLogs?.reduce((acc, log)=>({
        calories: acc.calories + (log.calories || 0),
        protein: acc.protein + (log.protein || 0),
    ...
    const lastAssistantMessage = chatHistory.length > 0 ? chatHistory.slice().reverse().find((msg)=>msg.role === 'assistant') : null;
    let contextAnalysis = '';
...
📊 Итого: 900 ккал`;
    ...
    if (!cacheHit) {
      ...
      const data = await callOpenAIWithRetry('https://api.openai.com/v1/chat/completions', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${OPENAI_API_KEY}`,
          'Content-Type': 'application/json'
      ...
      if (shouldCache && cacheKey) {
        await saveToCache(cacheKey, 'recipe', {
          request,
          calories: plan?.calories,
          preferences: userPreferences.map((p)=>p.item)
        }, recommendation, 86400 // TTL 24 часа для рецептов (они меняются реже)
        );
      }
    }
    ...
    await sendMessage(chatId, `📋 ${recommendation}`, aiResponseActionsKeyboard(hasMultipleMeals), 'Markdown', messageId);
 ...
 */ async function showNotificationsMenu(chatId, dbUserId) {
  try {
    ...
    let { data: settings } = await supabase.from('notification_settings').select('*').eq('user_id', dbUserId).single();
    // Если настроек нет, создаем с дефолтными значениями
    if (!settings) {
      const { data: newSettings } = await supabase.from('notification_settings').insert({
        user_id: dbUserId,
 ...
 */ async function toggleNotifications(chatId, dbUserId, type) {
  try {
    // Получаем текущие настройки
    const { data: settings } = await supabase.from('notification_settings').select('*').eq('user_id', dbUserId).single();
    if (!settings) {
 ...
 */ async function showHelpMenu(chatId, dbUserId) {
 ...
 */ async function showProfileMenu(chatId, dbUserId) {
  try {
    // ⚡ PHASE 2 OPTIMIZATION: 1 запрос вместо 2
    ...
    const activityLevel = profile.activity_level === 'sedentary' ? 'Низкая' : profile.activity_level === 'lightly_active' ? 'Легкая' : profile.activity_level === 'moderately_active' ? 'Средняя' : profile.activity_level === 'very_active' ? 'Высокая' : 'Очень высокая';
    ...
    try {
      const { data: streakStats, error: streakError } = await supabase.rpc('get_user_streak_stats', {
        p_user_id: dbUserId
      }).single();
      ...
      if (!streakError && streakStats) {
        ...
        if (streakStats.achievements && streakStats.achievements.length > 0) {
          ...
          for (const achievement of recentAchievements){
            const icon = achievement.type === 'streak_3' ? '🔥' : achievement.type === 'streak_7' ? '⭐' : achievement.type === 'streak_14' ? '💫' : achievement.type === 'streak_30' ? '🌟' : achievement.type === 'streak_100' ? '👑' : achievement.type === 'total_logs_10' ? '📊' : achievement.type === 'total_logs_50' ? '📈' : achievement.type === 'total_logs_100' ? '🎯' : '🏆';
 ...
 */ async function showUserPreferencesMenu(chatId, dbUserId) {
  try {
    ...
    if (preferences.length > 0) {
      keyboard.push([
        {
          text: "🗑 Очистить все предпочтения",
          callback_data: "clear_all_preferences"
        }
      ]);
    }
    keyboard.push([
      {
        ...
        callback_data: "clear_chat_history"
      }
    ], [
      {
        text: "🔙 Назад",
 ...
 */ async function showSubscriptionMenu(chatId, dbUserId) {
  try {
    // Получаем информацию о подписке
    const subscriptionData = await getSubscriptionInfo(dbUserId);
    const subscriptionInfo = Array.isArray(subscriptionData) ? subscriptionData[0] : subscriptionData;
 ...
 */ async function manageMeals(chatId, dbUserId) {
  try {
    ...
    const { data: logs } = await supabase.from('food_logs').select('*').eq('user_id', dbUserId).gte('logged_at', `${startDate}T00:00:00`).order('logged_at', {
      ascending: false
    });
    ...
    Object.keys(logsByDate).sort().reverse().forEach((date)=>{
      ...
      logsByDate[date].forEach((log)=>{
        ...
        const desc = log.description.length > 100 ? log.description.substring(0, 100) + '...' : log.description;
        ...
        keyboard.inline_keyboard.push([
          {
            text: `✏️ #${mealIndex}`,
            callback_data: `edit_meal_${log.id}`
          },
          {
            text: `🗑 #${mealIndex}`,
            callback_data: `delete_meal_${log.id}`
          }
        ]);
        message += '\n';
      });
 ...
 */ async function deleteMeal(chatId, dbUserId, mealId) {
  try {
    // Получаем информацию о приеме
    const { data: meal } = await supabase.from('food_logs').select('*').eq('id', mealId).eq('user_id', dbUserId).single();
    if (!meal) {
      await sendMessage(chatId, "❌ Прием пищи не найден");
      return;
    }
    const time = new Date(meal.logged_at).toLocaleTimeString('ru-RU', {
      hour: '2-digit',
      minute: '2-digit'
    });
    // Показываем подтверждение с деталями
    await sendMessage(chatId, `⚠️ **Подтвердите удаление**\n\n` + `⏰ ${time}\n` + `📝 ${meal.description}\n` + `🔥 ${meal.calories} ккал | 🥩 Б:${meal.protein}г | 🥑 Ж:${meal.fats}г | 🍞 У:${meal.carbs}г\n\n` + `Это действие нельзя отменить.`, {
      inline_keyboard: [
        [
          {
            text: "✅ Да, удалить",
            callback_data: `confirm_delete_meal_${mealId}`
          },
          {
            text: "❌ Отмена",
            callback_data: "manage_meals"
 ...
 */ async function confirmDeleteMeal(chatId, dbUserId, mealId) {
  try {
    // Получаем информацию о приеме
    const { data: meal } = await supabase.from('food_logs').select('*').eq('id', mealId).eq('user_id', dbUserId).single();
    if (!meal) {
    ...
    // Удаляем
    const { error } = await supabase.from('food_logs').delete().eq('id', mealId).eq('user_id', dbUserId);
    if (error) {
      throw error;
    }
    const time = new Date(meal.logged_at).toLocaleTimeString('ru-RU', {
    ...
    await sendMessage(chatId, `✅ **Прием пищи удален**\n\n⏰ ${time}\n📝 ${meal.description}`, {
      inline_keyboard: [
        [
          {
            text: "📝 Мои приемы",
...
/**
 * Показать дневник
 */ async function showDiary(chatId, dbUserId) {
  try {
    // Получаем план
    const { data: plan } = await supabase.from('nutrition_plans').select('*').eq('user_id', dbUserId).eq('is_active', true).single();
    const { data: profile } = await supabase.from('user_profiles').select('*').eq('user_id', dbUserId).single();
...
🍞 ${round(plan.carbs - consumed.carbs)}г углеводов`;
    ...
    if (todayLogs && todayLogs.length > 0) {
      ...
      todayLogs.forEach((log, index)=>{
        const time = new Date(log.logged_at).toLocaleTimeString('ru-RU', {
          hour: '2-digit',
          minute: '2-digit'
        });
